...
- [ ] **Data Integrity**: Ensure any column data used in local calculations (e.g., ATR, EMA21 for Action Zone) are explicitly included in the final `DataWriter` output for auditability.

### Query Optimization & Scaling
- [x] **Address Default Result Limit (50)**: Increase the `limit` in the TradingView query to ensure a larger starting candidate pool. (Currently 150).
- [ ] **Move Filters Server-side**: Progressively move local filters to the TradingView `Query().where(...)` to reduce data transfer and over-filtering risks:
    ...
    - [ ] Move Earnings condition (Exclude if next 14 days).
    - [ ] Move Action Zone (Price within 1 ATR of EMA21 - requires cross-column math if supported).

### Technical Manual Alignment & Refinement
- [ ] **Refine Indicator Periods**: Update query columns and logic to match the manual's specific periods:
    - [ ] Use `ADX(13)` instead of default (14).
    - [ ] Use `Stochastics(8, 3)` instead of default.
- [ ] **Expand Trend Filters**: Add additional institutional "Lines-in-the-Sand" to the query:
    - [ ] Add `Close > SMA50`.
    - [ ] Add `Close > SMA100`.
- [ ] **Volume/Momentum Quality**: Add additional liquidity and momentum filters to the query:
    - [ ] Add `Relative Volume > 1.0` (Ensuring active participation).
    - [ ] Add `Change % > 0` (Confirming daily bullish bias).
- [ ] **Scale Scanning Capacity**: Increase query `limit` from 150 to 500 to capture a wider universe of stacked trends before applying the highly restrictive RSI(2) and Action Zone filters.
